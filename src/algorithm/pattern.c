...
static int upgrade_col[SEGMENT_MASK][UP_COL_SIZE];
static int count[SEGMENT_MASK];
static pattern_t pattern_memo[SEGMENT_MASK];
...
pattern_t to_pattern(int segment_value, bool check_forbid)
{
    ...
    pattern_t pat = pattern_memo[segment_value];
    if (!check_forbid && pat == PAT_TL) return PAT_5;
    if (!check_forbid && pat == PAT_ATL) return PAT_D4;
    return pat;
}
...
{
    assert(pattern_initialized);
    pattern4_t pat4 = pattern4_memo[x][y][u][v];
    if (!check_forbid && pat4 == PAT4_TL) return PAT4_WIN;
    if (!check_forbid && (pat4 == PAT4_44 || pat4 == PAT4_A33)) return PAT4_OTHERS;
    return pat4;
}
...
#define print_array(...)                      \
    for (int i = 0; i < SEGMENT_MASK; i++) {  \
        if (pattern_mem[i]) {                 \
            print_segment(decode_segment(i)); \
        }                                     \
    }
